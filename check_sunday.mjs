...
async function check() {
  const { data, error } = await supabase.from('posts').select('id, status, scheduled_for, created_at, text').order('created_at', { ascending: false }).limit(10);
  data.forEach(p => {
      console.log(`[${p.status}] ID: ${p.id.slice(0,5)} | Sched: ${p.scheduled_for} | Created: ${p.created_at} | Text: ${p.text.substring(0, 30).replace(/\n/g, ' ')}`);
  });
}
